...
def generate_content_with_api(data):
...
# Example usage for the upload route
json_file_path = input("Enter the path to your JSON file: ")
with open(json_file_path, 'r') as file:
    data = json.load(file)

response = generate_content_with_api(data)
model_response = response['candidates'][0]['content']['parts'][0]['text']
print(model_response)

# Example usage for the update route
update_data = {
    "requirements": ["New Requirement 1", "New Requirement 2"],
    "additionalFeatures": {"details": "New Additional Features"}
}

response_update = generate_content_with_api(update_data)
model_response_update = response_update['candidates'][0]['content']['parts'][0]['text']
print(model_response_update)
'''